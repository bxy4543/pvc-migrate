...
`reserve`, `copy`, `migrate`, and `migrate-pod` accept `--destination-capacity` because they create destination PVCs. Pass one value to apply it to every source PVC, or use explicit `source-pvc-name=capacity` entries for multiple PVCs. Plans and workflow-specific `status` commands show both source and destination capacities.

OpenEBS Local PV HostPath and static HostPath volumes are not constrained by the PVC request, so their declared capacity may be smaller than the files already stored. For these sources, pvc-migrate runs a short-lived, read-only usage Pod on the volume node. When `--destination-capacity` is omitted, the destination request is the larger of the source PV capacity and 120% of measured allocated bytes, rounded up to MiB. An explicit destination capacity remains authoritative but is rejected when it is below that safety minimum. The measured bytes and resolved destination capacity are frozen in the workflow plan and rechecked before final sync so post-planning growth fails before cutover instead of reaching ENOSPC.

For other storage backends, omitting the flag keeps each source PV capacity. The planner rejects a destination smaller than its source PV by default. Add `--allow-volume-shrink` only after confirming that the copied data fits in every smaller PVC. pvc-migrate accepts shrink usage only from a trusted adapter for a known storage-backend CRD; provisioned capacity is not treated as used bytes. When no trusted adapter is available, the plan is blocked unless `--skip-source-usage-check` explicitly accepts the risk. The current release has no trusted CRD usage adapter for OpenEBS LVM or S3 CSI. These flags apply only to a new session and cannot change an existing session. `rename` and `move` preserve PVC identity and do not expose capacity flags.
